...
describe("vite integration", () => {
  ...
  test(
    "SPA build emits pipeline assets and the bundle references them",
    async () => {
      await build({
        root: fixtureDir,
        logLevel: "warn",
      ...
      const files = (
        await readdir(buildOut, { recursive: true })
      ).map((f) => String(f).replace(/\\/g, "/"));
      expect(files).toContain("index.html");

      const html = await readFile(resolve(buildOut, "index.html"), "utf-8");
      const scriptMatch = html.match(/src="([^"]+\.js)"/);
      expect(scriptMatch, "html should reference the entry script").not.toBeNull();
  ...
  test(
    "SPA build inlines ?raw imports as strings",
    async () => {
      await build({
        root: fixtureDir,
        logLevel: "warn",
  ...
  test(
    "dev server resolves imports to dev URLs and serves outputs over HTTP",
    async () => {
      ...
      try {
        ...
        const virtual = await server.transformRequest(
          "\0assetpipe:/hello.txt",
        );
  ...
  test(
    "dev server invokes handleReload when an input asset changes",
    async () => {
  ...
  test(
    "custom prefix scopes which imports the plugin claims",
    async () => {
      const server = await createServer({
        root: fixtureDir,
        logLevel: "warn",
