...
const build = (name) =>
  new Promise((resolve, reject) => {
    ...
    const proc = exec("npm run build", {
      cwd: `${packagesDir}/${packages[name].dir}`,
    });
    const pipe = (stream) =>
      createInterface({ input: stream, crlfDelay: Infinity }).on(
        "line",
        (line) => process.stdout.write(`${tag} | ${line}\n`),
      );
    pipe(proc.stdout);
    pipe(proc.stderr);
    proc.on("close", (code) =>
      code === 0
        ? resolve()
        : reject(new Error(`${name} build failed (exit ${code})`)),
    );
  });
